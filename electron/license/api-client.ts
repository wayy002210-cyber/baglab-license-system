...
type LicenseFetcher=(input:string,init?:RequestInit)=>Promise<Response>;
export class LicenseApiClient{
  constructor(private readonly origin:string,private readonly fetcher:LicenseFetcher=(input,init)=>fetch(input,init),private readonly delay:(ms:number)=>Promise<void>=(ms)=>new Promise(resolve=>setTimeout(resolve,ms))){ }
  async post<T>(path:string,input:unknown):Promise<T>{
    let last:unknown;
    for(let attempt=0;attempt<3;attempt++)try{
      const response=await this.fetcher(new URL(path,this.origin).toString(),{method:"POST",headers:{"Content-Type":"application/json"},body:JSON.stringify(input),signal:AbortSignal.timeout(8_000)});
...
export function createElectronLicenseApiClient(origin:string,electronFetch:LicenseFetcher,delay?:(ms:number)=>Promise<void>){
  return new LicenseApiClient(origin,electronFetch,delay);
}
