...
export function createProjectTasks(input:Input):GenerationTask[]{
 ...
 const create=input.database.transaction(()=>input.projectIds.map((projectId,index)=>{
  ...
  const seed=input.seed+index;const explicitlySelected=input.stylePresetSelection&&input.stylePresetSelection!=="__random__"?input.stylePresets.find(item=>item.id===input.stylePresetSelection):null;const preset=explicitlySelected??(input.stylePresets.length?input.stylePresets[Math.abs(seed)%input.stylePresets.length]:null);
