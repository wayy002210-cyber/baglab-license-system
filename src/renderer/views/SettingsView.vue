...
type CredentialName = "bailian" | "minimax";
type Health = "unknown" | "connected" | "failed";

const credentialStatus = reactive({ bailian: false, minimax: false });
const secrets = reactive({ bailian: "", minimax: "" });
const health = reactive<Record<CredentialName, Health>>({
  bailian: "unknown",
  minimax: "unknown"
});
const savingCredential = ref<CredentialName | null>(null);
...
async function saveCredential(name: CredentialName): Promise<void> {
  ...
  try {
    ...
    health[name] = "unknown";
    await load();
    ElMessage.success("密钥已安全保存到 Windows 凭据管理器");
  } catch (error) {
    ElMessage.error(error instanceof Error ? error.message : "保存密钥失败");
  } finally {
    savingCredential.value = null;
  }
}

async function clearCredential(name: CredentialName): Promise<void> {
...
async function testCredential(name: CredentialName): Promise<void> {
  testingCredential.value = name;
  try {
    if (name === "bailian") {
      const result = await window.autocut.testBailianConnection(
        copyModel.defaultModel
      );
      health.bailian = "connected";
      ElMessage.success(`百炼连接正常，模型 ${result.model} 可调用`);
    } else {
      const result = await window.autocut.testMinimaxConnection();
      health.minimax = "connected";
      ElMessage.success(`MiniMax 连接正常，读取到 ${result.voiceCount} 个在线音色`);
    }
  } catch (error) {
    health[name] = "failed";
    ElMessage.error(
      error instanceof Error ? error.message : `${name} 连接测试失败`
    );
  } finally {
    ...
    <section class="settings-grid">
      <article class="surface settings-card">
        <header>
          ...
          <div>
            <h3>AI 服务密钥</h3>
            <p>密钥仅保存在 Windows 凭据管理器，不写入数据库和日志</p>
          </div>
        </header>
        <div
          v-for="name in (['bailian', 'minimax'] as const)"
          :key="name"
          class="credential"
        >
          <div class="credential-title">
            <strong>{{ name === "bailian" ? "阿里云百炼" : "MiniMax" }}</strong>
            <el-tag :type="credentialStatus[name] ? 'success' : 'info'">
              {{ credentialStatus[name] ? "已配置" : "未配置" }}
            </el-tag>
          </div>
          <el-input
            v-model="secrets[name]"
            type="password"
            show-password
          ...
          >
            <template #append>
              <el-button
                :loading="savingCredential === name"
                @click="saveCredential(name)"
              >
                保存
              </el-button>
            </template>
          </el-input>
          <div v-if="credentialStatus[name]" class="credential-actions">
            <el-button
              :loading="testingCredential === name"
              @click="testCredential(name)"
            >
              {{ name === "bailian" ? "测试百炼与当前模型" : "测试 MiniMax 与音色库" }}
            </el-button>
            <el-tag
              v-if="health[name] !== 'unknown'"
              :type="health[name] === 'connected' ? 'success' : 'danger'"
            >
              {{ health[name] === "connected" ? "连接正常" : "连接失败" }}
            </el-tag>
            <el-button link type="danger" @click="clearCredential(name)">
              清除密钥
            </el-button>
          </div>
        </div>
      ...
      <article class="surface settings-card media-card">
        <header>
          ...
          <div><h3>成片与媒体</h3><p>1080×1920 / 30fps / H.264 + AAC</p></div>
        </header>
        <el-form label-position="top">
          <el-form-item label="成片目录">
            <el-input v-model="media.outputDirectory">
              <template #append><el-button @click="selectPath('output')">选择</el-button></template>
            </el-input>
          </el-form-item>
          <el-form-item label="工作目录">
            <el-input v-model="media.workDirectory">
              <template #append><el-button @click="selectPath('work')">选择</el-button></template>
            </el-input>
          </el-form-item>
          <div class="form-row">
            <el-form-item label="编码器">
              <el-select v-model="media.encoder">
                <el-option label="自动检测" value="auto" />
                <el-option label="NVIDIA NVENC" value="h264_nvenc" />
                <el-option label="Intel QSV" value="h264_qsv" />
                <el-option label="AMD AMF" value="h264_amf" />
                <el-option label="CPU libx264" value="libx264" />
              </el-select>
            </el-form-item>
            <el-form-item label="码率 Mbps">
              <el-input-number v-model="media.videoBitrateMbps" :min="1" :max="50" />
            </el-form-item>
          </div>
          <el-form-item label="默认字幕字体"><el-input v-model="media.fontFamily" /></el-form-item>
          <el-form-item label="默认背景音乐">
            <el-input :model-value="media.bgmPath || ''" readonly>
              <template #append><el-button @click="selectPath('bgm')">选择</el-button></template>
            </el-input>
          </el-form-item>
          <el-form-item label="默认 BGM 音量">
            <el-slider v-model="media.bgmVolume" :min="0" :max="1" :step="0.01" />
          </el-form-item>
          <el-button type="primary" :loading="savingMedia" @click="saveMedia">
            保存媒体设置
          </el-button>
        </el-form>
      </article>
      ...
      <article class="surface settings-card copy-card">
        <header>
          ...
          <div>
            <h3>文案模型与参考脚本</h3>
            <p>通过百炼调用通义或 DeepSeek，参考脚本仅保存在本机</p>
          </div>
        </header>
        <el-form label-position="top">
          <div class="form-row">
      ...
      <article class="surface settings-card">
        <header>
          ...
          <div>
            <h3>故障诊断</h3>
            <p>诊断包不包含密钥、Cookie、Authorization 或任务产物</p>
          </div>
        </header>
